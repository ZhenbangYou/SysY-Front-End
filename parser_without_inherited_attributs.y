...
VarDef        : IDENT ConstExpList {
                    ...
                    {
                        $$=new Var();
                        if(parser.top->prev==NULL)//global variable
                            emit(((Var*)$$)->getname()+"=0");
                    }
                    else//array variable
                    {
...
Stmt          : LVal '=' Exp ';'
              ...
              | IF '(' Cond ')'
              {
                  $1=new IfStmt(*(int*)$3,NewLabel(),NewLabel());
                  emit("goto l"+to_string(((IfStmt*)$1)->False));
              ...
              | WHILE
              {
                  $1=new WhileLoop(NewLabel(),0,NewLabel());
                  parser.while_stack.push_back((WhileLoop*)$1);
                  emitLabel(((WhileLoop*)$1)->Begin);
              } 
              '(' Cond ')'
              {
                  emit("goto l"+to_string(((WhileLoop*)$1)->After));      

                  ((WhileLoop*)$1)->Body=*(int*)$4;
...
LAndExp       : EqExp 
                {
                    $$=new int;//false label
                    *(int*)$$=NewLabel();
                    emit("if "+((Var*)$1)->getname()+"== 0 goto l"+to_string(*(int*)$$));                    
                }
              | LAndExp AND EqExp
                {
                    $$=$1;//false label
                    emit("if "+((Var*)$3)->getname()+"== 0 goto l"+to_string(*(int*)$$));                    
                }
              ;
LOrExp        : LAndExp 
              {
